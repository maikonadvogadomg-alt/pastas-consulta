...
const ACTION_PROMPTS: Record<string, string> = {
  ...
  minuta:
    "Elabore MINUTA COMPLETA: Enderecamento, Qualificacao, Fatos, Direito e Pedido. Fundamentacao juridica robusta.\n\nINFORMACOES:\n{{textos}}",
  analisar:
...
<div class="paper">${data.html}</div>
  ...
  app.post("/api/jurisprudencia/buscar", async (req, res) => {
    try {
      ...
      const tribunaisList =
        Array.isArray(tribunais) && tribunais.length > 0
          ? tribunais.join(", ")
...
Tribunais desejados: ${tribunaisList}
...
- Use APENAS jurisprudências reais e verificáveis — nunca invente números de processo, relatores ou datas
- Retorne entre 4 e 8 resultados relevantes
- Distribua entre os tribunais solicitados quando possível
- Ementa deve ser o texto real/oficial (pode ser resumida se muito longa)
- Se não houver jurisprudência real conhecida de um tribunal, não inclua resultados fictícios daquele tribunal`;
      ...
      res
        .status(500)
        .json({ message: "Erro ao buscar jurisprudências: " + e.message });
    }
  });
    ...
    try {
      ...
      if (html) {
        ...
        const htmlToWordXml = (htmlStr: string): string => {
          ...
          const tagRegex =
            /<(p|blockquote|h[1-6]|div)([^>]*)>([\s\S]*?)<\/\1>/gi;
          let match;
        ...
        if (selectedEmentas.length > 0) {
          ementasForSystem =
            ...
            selectedEmentas
              .map(
                (e, i) =>
                  `EMENTA ${i + 1} [${e.categoria}] - ${e.titulo}:\n${e.texto}`,
              )
              .join("\n\n") +
  ...
  app.post("/api/ai/refine", async (req, res) => {
    try {
      ...
      if (!previousResult || !instruction) {
        return res
          .status(400)
          .json({ message: "Resultado anterior e instrução são obrigatórios" });
      }
...
REGRAS FIXAS: Mantenha dados pessoais exatos. Texto puro sem markdown. Use historico da conversa. Nao invente fatos. Se instrucao for vaga ou emocional, retorne o documento atual integralmente.`;
      ...
      if (Array.isArray(chatHistory) && chatHistory.length > 0) {
        ...
        geminiMessages.push({
          role: "model",
          parts: [{ text: previousResult }],
        });
        geminiMessages.push({ role: "user", parts: [{ text: instruction }] });
      }

      console.log(
        `[AI Refine] Effort: ${effort}, Verbosity: ${verb}, MaxTokens: ${refineMaxTokens}`,
      );
        ...
        for (const part of candidate.content.parts) {
          ...
          if (p.codeExecutionResult) {
            ...
            if (result.outcome === "OUTCOME_OK") {
              output =
                result.output ||
                "(sem saída — use print() para ver resultados)";
            } else {
